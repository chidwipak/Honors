...
class RealBiomedCLIPEvaluator(RealModelEvaluator):
    """Real BiomedCLIP evaluator using the actual model files"""
    
    def load_model(self):
        logger.info(f"Loading BiomedCLIP model from {self.model_path}")
        try:
            # Try to load as a local model first
            if os.path.exists(os.path.join(self.model_path, "open_clip_pytorch_model.bin")):
                # This is a local BiomedCLIP model
                from transformers import CLIPConfig
                config = CLIPConfig.from_pretrained(self.model_path)
                self.model = CLIPModel(config)
                
                # Load the state dict
                state_dict = torch.load(os.path.join(self.model_path, "open_clip_pytorch_model.bin"), 
...
class RealMedGemmaEvaluator(RealModelEvaluator):
    ...
    def load_model(self):
        ...
        try:
            ...
            hf_token = os.environ.get('HF_TOKEN') or os.environ.get('HUGGINGFACE_HUB_TOKEN')
            self.model = AutoModelForCausalLM.from_pretrained("google/gemma-2b", token=hf_token)
    ...
    def load_model(self):
        ...
        try:
            ...
            if os.path.exists(os.path.join(self.model_path, "config.json")):
                ...
                self.processor = AutoProcessor.from_pretrained(self.model_path, trust_remote_code=True, token=hf_token)
            else:
                # Try HuggingFace model name
                self.model = AutoModelForVision2Seq.from_pretrained(
                    self.model_path,
            ...
            hf_token = os.environ.get('HF_TOKEN') or os.environ.get('HUGGINGFACE_HUB_TOKEN')
            self.model = AutoModelForVision2Seq.from_pretrained("Qwen/Qwen-VL", trust_remote_code=True, token=hf_token)
            self.processor = AutoProcessor.from_pretrained("Qwen/Qwen-VL", trust_remote_code=True, token=hf_token)
            self.model.to(self.device)
